...
7. **写回颗粒度**：每条写回要务必须是一个独立的小标题/有序列表项。同一 OKR 行有多件事时，
   ...
   MIT 样式沿用历史表格：任务标题后追加红色 `MIT`。计划阶段**不追加 `✅`**——
   写回时标完成等于给之后的复盘伪造完成证据。
...
10. **retro review 写回**：weekly / biweekly 模式可以在**被复盘周期**（刚结束的那一列）
    ...
    周期的复盘，不写进正在规划的目标周。review 的参考顺序是：① Daily OS 补充上下文里
    「Local Cycle Retro」段落中被复盘周期的那一段——用户现在在 Daily OS 的 Cycles 页
    手写复盘，那里是最新的一份；② 同一 retro 单元格中已有的 `状态`、`做得好/做的好`、
    `待改进`（飞书上的可能为空或是旧稿）；③ 相邻要务完成状态和其余 Daily OS 补充上下文。
    这是第 9 条「Daily OS 上下文不得覆盖表格事实」的唯一例外，且只适用于 retro。
    review 控制在 350 个中文字符以内，只写复盘结论，不写来源说明或内部推理。
