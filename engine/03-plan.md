# Engine 03 — 生成下周计划

基于分析结果，生成结构化的下周要务列表。

## 输入

- `engine/02-analyze.md` 的输出结构
- 当前 `🐶 重点OKR` 定义（优先级、KR 进度）
- 用户感受（影响计划强度）
- 当前模式（weekly / biweekly / quarterly）

## 🐶 重点OKR约束

下周 KR / 要务必须来自 Feishu Weekly 文档的 `🐶 重点OKR` 章节：

- 不得为 `🐶 重点OKR` 中不存在的 Objective / KR 新建 KR。
- 若 watch-list 或 todos 中有事项值得推进，只能挂到已有 KR 下；挂不上则放入
  `[如有余力]`，并标注"非重点OKR"。
- 如果 `🐶 重点OKR` 章节缺失或无法解析，停止生成计划，提示用户修正文档。
- 输出中的 KR 名称应保留 Feishu 原文或稳定简称，方便写回表格后人工核对。

## 计划生成规则

### 规则 0：先应用任务量预算

生成下周计划前，先读取 `config.yaml` 的 `planning` 配置。

默认策略：

```yaml
planning:
  workload_mode: normal
  weekly_task_budget:
    mit: 1
    p1: 4
    p2: 3
  min_total_items: 6
  max_total_items: 10
  min_okr_rows_touched: 4
```

任务量预算只影响内部候选池和写回计划，不在最终卡片里展示。

- `conservative`：4-6 条，适合低精力/高外部压力周。
- `normal`：6-10 条，默认强度。
- `ambitious`：8-12 条，适合用户明确希望加量时。

如果 Feishu 上周未闭环事项不足以达到 `min_total_items`：

1. 先补充同一 OKR 行内的未闭环 carry-over。
2. 再从 Daily OS 输入包中的 Linear、todo inbox、vault daily、recent daily memory
   选择能挂到现有 `🐶 重点OKR` 行的候选。
3. 挂不到现有 OKR 行的候选不得进入写回计划。

最终用户可见输出只展示计划结果；不要展示来源、证据名、任务量预算、row_index 或内部推理。

### 规则 1：严格限制 MIT 数量

**每周只能有 1 个 MIT**（Most Important Thing）。

MIT 选择标准（按优先级，**有 metadata 时优先用 metadata**）：

#### 有 metadata（vault.enabled = true 且 metadata 存在）

1. `urgency = overdue` 且 priority ∈ {P0, P1}：必须升级为 MIT，否则继续滞后
2. `urgency = this_week` 且 priority = P0 且进度 <50%：强候选 MIT
3. 上周 MIT 未完成且 deadline 未过：延续为本周 MIT
4. `urgency = next_two_weeks` 且 P0：候选 MIT（有提前量）
5. 都不满足时，按规则"P0 中进度最落后的 KR"

> phantom / parked 状态的 KR **永远不进 MIT 候选**，除非用户在 dialog 里显式启用。

#### 无 metadata（fallback，原有逻辑）

1. 上周已标注 MIT 但未完成 → 延续为本周 MIT
2. P0 OKR 中进度最落后的 KR → 提升为 MIT
3. 有明确截止时间压力的 KR（从描述里关键词识别）

若上周有 2+ 个 MIT 且完成率低，本周主动收窄为 1 个，并在输出中说明原因。

### 规则 1.5：Phantom KR 自动剔除（仅有 metadata 时）

`status = phantom` 的 KR 不进入下周要务建议。**第二次连续出现该状态时**，在输出末尾加一段提示：

```
🔍 **下季度建议**
以下 KR 已连续被标记为 phantom（无 deadline 且未启动）：
- O3_KR4: 完成自己的 portfolio（标记于 2026-05-04）
是否在下季度规划时移除或降级？
```

### 规则 2：按 OKR 优先级分配任务量

```
P0 OKR → 每个最多 2 条要务
P1 OKR → 每个最多 1 条要务
长期/家庭类 → 不占周要务槽，除非有具体行动
```

总要务数建议控制在 **6-8 条**（含 MIT）。超过 8 条时主动裁剪低优先级条目。

### 规则 3：要务粒度要"可执行"

- ❌ 太模糊：`推进作品集`
- ✅ 足够具体：`完成作品集个站首页移动端适配并部署`

每条要务应在 1-3 小时内可完成，或有明确的检验标准（✅/⭕️）。

### 规则 3.5：延续性优先——照搬、按 retro 调整、或留空

对上一周期未完成、本期仍要继续的要务：

- **默认逐字照搬**上期原文（含 Linear 编号），不要换措辞重新编写一遍。改写不产生信息，只制造"看起来是新任务"的错觉。
- **仅当** Review / retro 中对该 KR 有明确反馈（如：太重、被阻塞、策略要换）时才修改，且修改必须**体现该反馈**：减少数量、拆成更小一步、或按 retro 描述重新安排切入点。禁止只换说法内容不变。
- **例外：Linear 备注比条目新时必须改写**。条目标注了 issue 编号，且 Daily OS context 的
  `## Linear Issue Notes` 里该 issue 的描述或备注与条目文案冲突（目标值、金额、范围、前置
  条件已经变了）时，按备注改写条目，用备注里的最新事实。这一条**优先于照搬**——照搬的目的
  是不丢信息，不是保留过时数字。issue 标题往往停在创建时的说法，备注才是执行事实。
- **备注把决定推迟到某个时点时，不要替它做决定**。备注写「等 9/1 快照后再定金额」这类
  话，说明数字还没定；要务就写成推进到那个决策点的动作，不要写死一个备注没给的数字或结论。
- **判断不了怎么安排的 KR 行，留空不写**。空着比编一条凑数好。

> 照搬是默认动作，不是唯一动作。上一周期的要务列是**种子**，不是**边界**——只照搬会让计划永远等于上期，新开的活进不来、已收尾的活出不去。规则 3.5b 负责补上这两个方向。

### 规则 3.5b：Linear 覆盖核对（双向）

prompt 里若出现 `# Linear 覆盖核对` 段落，它由程序比对 Linear 与飞书要务列生成，**必须逐条处理，不允许整段跳过**。

**方向一 · 进行中但要务列没有的 issue**

这些 issue 在 Linear 上已经开工（In Progress / In Review），但上期和本期要务列都没提到——说明它们从未进入计划，而不是被判断过后放弃的。

- 逐条二选一：纳入本期要务（写成一条可执行要务，按规则 3.6 标注编号），或本期明确不做。
- 决定不做的，在草稿正文用一句话说明原因，不写进 writeback_plan。
- 挂不到任何第一列 OKR 行的，同样不进 writeback_plan（规则 3 的约束优先）。

**方向二 · 要务列还挂着、但 Linear 已经收尾的 issue**

飞书单元格的 ✅ 靠手动标记，漏标很常见；Linear 的状态才是执行事实。

- 这些条目**不要**照搬进新周期要务列。
- 复盘正文里按已完成计入，不要继续当作未闭环事项。
- 确实还有后续工作的，写一条**描述后续动作**的新要务，不要复制原文。

> 写回阶段还有一层程序兜底：引用了已收尾 issue 的条目会被直接剔除，并记录在 run 的 `evidence.linear_coverage.retired_from_plan` 里。

### 规则 3.6：挂 Linear 编号

- 输入证据（daily-os input pack 的 Linear 段）中若存在与该要务对应的 issue，在条目末尾以 `(LEO-97)` 形式标注编号；只写编号，不写标题。多个编号用空格分隔。
- 只在**确定对应**时标注；对应关系拿不准就不标，禁止猜编号。
- 写回飞书时编号会自动渲染为可点击的 Linear 链接。

### 规则 4：结合用户感受调整强度

| 用户感受 | 计划调整 |
|---|---|
| 精力充沛、情绪稳定 | 正常计划强度 |
| 情绪低落、外部压力大 | 减少 1-2 条要务，MIT 保留，其余降级 |
| 极度倦怠 | 只保留 MIT，其余全部标记为"如有余力" |
| 生活重大变化（旅行/考试等） | 主动标注该周为"轻量周"，计划减半 |

### 规则 5：渐进自动化原则

初期（V1）：每条要务都基于用户提供的 retro 推导，AI 不凭空生成。
成熟期（V2+）：AI 可根据历史执行模式预判下周要务，用户只需确认或调整。

在本工作流中，V1/V2 都必须先满足 `🐶 重点OKR` 约束；历史模式只能决定同一 KR
下的具体动作粒度和优先级，不能创造新的 KR。

### 规则 6：Phantom KR 自动剔除（仅有 metadata 时）

见 `engine/02-analyze.md` 的 phantom_kr_warning 诊断。`status = phantom`
的 KR 不进入下周要务建议；连续两次出现时在输出末尾提示用户考虑下季度移除。

### 规则 7：Todos 折入"如有余力"（仅 vault.todos 已配置）

来自 `engine/07-todos.md` 的 todos 数据按以下规则折入下周计划：

#### 候选优先级

1. `urgency = overdue` 的 todos：必须列入（用户标 [urgent] 或 DDL 已过）
2. `urgency = this_week` 的 todos：建议列入
3. 其他 open todos：仅在状态分数高且 KR 要务 ≤ 5 条时考虑

#### 强制约束

- todos 写入 `如有余力` 区块，**不抢 MIT 槽位**
- todos 不计入 6-8 条要务总数上限（独立计数）
- `[blocked]` / `[parked]` 的 todos 永远不主动建议
- 单次 review 折入 todos 上限 **2 条**（避免清单膨胀）

#### 强度调整（叠加规则 4）

| 用户感受 | todos 折入数 |
|---|---|
| 精力充沛、情绪稳定 | 上限 2 条 |
| 情绪低落、外部压力大 | 上限 1 条（仅 overdue + urgent） |
| 极度倦怠 | 0 条（todos 全部隐藏，只显示背景统计） |

## 输出格式

生成供写回飞书的结构化计划：

```
MIT 🔴：{要务描述}

KR1（{🐶 重点OKR中的OKR名称}）：
1. {要务1}  MIT/—
2. {要务2}

KR2（{🐶 重点OKR中的OKR名称}）：
1. {要务}

[如有余力]
- {KR 要务 - 低优先级}
- {todos 折入项，标 📝} ← 来自 todos.md，可选

KR3（{OKR名称}）：
1. {要务}

[如有余力]
- {低优先级要务}
```

## biweekly / quarterly 模式的额外输出

### biweekly 追加：节奏校正建议
```
## 双周趋势
- {KR} 已连续 {N} 周未完成，建议：{具体调整}
- 本双周 MIT 完成率：{x}%，{高于/低于}上双周
```

### quarterly 追加：方向校准建议
```
## 季度诊断
- 与使命宣言的偏离：{描述}
- 建议降级的 OKR：{列表}
- 建议升级关注的 OKR：{列表}
- 下季度重点：{1-2 句}
```
