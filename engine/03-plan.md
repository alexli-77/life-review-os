...
### 规则 3.5：延续性优先——照搬、按 retro 调整、或留空

对上一周期未完成、本期仍要继续的要务：

- **默认逐字照搬**上期原文（含 Linear 编号），不要换措辞重新编写一遍。改写不产生信息，只制造"看起来是新任务"的错觉。
- **仅当** Review / retro 中对该 KR 有明确反馈（如：太重、被阻塞、策略要换）时才修改，且修改必须**体现该反馈**：减少数量、拆成更小一步、或按 retro 描述重新安排切入点。禁止只换说法内容不变。
- **判断不了怎么安排的 KR 行，留空不写**。空着比编一条凑数好。

### 规则 3.6：挂 Linear 编号

- 输入证据（daily-os input pack 的 Linear 段）中若存在与该要务对应的 issue，在条目末尾以 `(LEO-97)` 形式标注编号；只写编号，不写标题。多个编号用空格分隔。
- 只在**确定对应**时标注；对应关系拿不准就不标，禁止猜编号。
- 写回飞书时编号会自动渲染为可点击的 Linear 链接。
