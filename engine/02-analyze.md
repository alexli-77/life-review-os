...
- OKR 定义（只从 Weekly 文档的 `🐶 重点OKR` 章节提取）
- 最近 N 周执行数据（只从 `🐶` Weekly 表格中的要务列 + retro 列提取）
- 用户当次输入的 retro 和感受
- 分析框架（从 `frameworks/{framework}.md` 加载）

## 权威性规则

- Feishu `🐶` 表格里的每周要务是 review 的事实来源。
- 用户当次输入只能解释原因、补充感受、修正明显漏记；不能替代缺失的表格数据。
- 只有 `🐶 重点OKR` 中出现的 Objective / KR 能进入 KR 完成率和下周 KR 规划。
- vault watch-list / todos 只能进入独立背景或"如有余力"，不能计入 KR 完成率。

## 分析步骤

### Step 1：建立 OKR 对照表

从 `🐶 重点OKR` 章节中提取每个 OKR 的：
- Objective 描述
- 每个 KR 的描述
- P0/P1 优先级
- 是否标注 MIT
...
对每个 KR，在 `🐶` 表格最近 N 周的每周要务中找到对应的要务项，判断：
...
校正边界：
- 如果用户说"已完成但没记录"，输出应标为"用户补充完成，Feishu 未记录"。
- 不得把用户补充直接当作 Feishu 表格完成事实。
- 若 Feishu 表格为空或缺少本周列，结论必须指出"数据源缺失"，并停止自动写回。
